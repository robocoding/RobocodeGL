...
public class BattleView {
	...
	public int getWidth() {
		return 800;// getCanvas().getWidth();
	}

	public int getHeight() {
		return 600;//getCanvas().getHeight();
	}
	...
	public BufferedImage getScreenshot() {
		// BufferedImage screenshot = getCanvas().getGraphicsConfiguration().createCompatibleImage(getWidth(), getHeight());
		//
	...
	private void update(ITurnSnapshot snapshot) {
		...
		//
		// if (windowManager.isIconified() || !getCanvas().isDisplayable() || (getWidth() <= 0) || (getHeight() <= 0)) {
		// 	return;
		// }
		//
